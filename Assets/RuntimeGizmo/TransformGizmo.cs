...
namespace RuntimeGizmos
{
    ...
    public class TransformGizmo : MonoBehaviour
    {
            ...
            if (Input.GetMouseButtonDown(0))
            {
                ...
                if (Physics.Raycast(myCamera.ScreenPointToRay(Input.mousePosition), out hitInfo))
                {
                    ...
                    if (hitInfo.transform != target)
                    {
                        ...
                        HighlightSelectedGizmo(oldTarget, target);
                    }
                }
            }
        }
        ...
        private void HighlightSelectedGizmo(Transform oldTarget = null, Transform newTarget = null)
        {
            if (oldTarget)
                oldTarget.GetComponent<InteractionGizmo>().highlighter.DeSelect();
            if (newTarget)
                newTarget.GetComponent<InteractionGizmo>().highlighter.Select();
        }
